...
Obelisk stores Claude Code, Codex, Kimi Code, Pi, and DeepSeek Harness
transcripts in the same schema.
...
- DeepSeek Harness rows use `source='deepseek'`; session and message IDs are
  prefixed with `deepseek:`. Subagent logs fold into their root parent session
  as sidechain messages and are recorded in `subagents`.
