...
- `sessions(opts?)` -- session rows, newest first. `project` is a SQL `LIKE` pattern. `sessionId`/`sessions` also accept a fragment such as a raw provider uuid and return every containing session when the fragment is ambiguous. `message_count` counts the visible canonical transcript; inactive and hidden records are excluded. The invoking session row carries `is_invoking: true`.
